...
# - they launch on port 13000, 13001, ...
# - they star rpc on port 6100, 6101, ...
# - by collecting the nodes nodeUrl, they get connected to each other
# - if enode has no IP, `<local_IP>` is substituted
# - if `<network_id>` is not 2020, they will not connect to a default client,
...
cmd="bash ./g420cluster.sh ~/leagues/swarm/ $network_id $N ''  --vmodule=netstore=6,depo=6,forwarding=6,hive=5,dpa=6,dpa=6,http=6,syncb=6,syncer=6,protocol=6,swap=6,chequebook=6 --verbosity=5  --vmdebug=false --maxpeers=20 --dev --shh=false --nodiscover --ipcexp"
echo $cmd
eval $cmd